...
def test():
    ...
    leg, y = zip(
        ...
        (r'Ga $\sqrt{2\ln 2}\tau$',
            time_function('gaussian', t, tau*np.sqrt(2 * np.log(2)))),
        (r'Decon $\sqrt{2}\tau$',   brune2gauss(x, dt, tau, tau * np.sqrt(2))),
        # ('Hann',                   time_function('hann', t, tau)),
        # ('Ricker1',
        #   time_function('ricker1', t - 0.5 * dt, tau).cumsum() * dt),
        # ('Ricker2',
        #   time_function('ricker2', t - dt, tau).cumsum().cumsum() * dt * dt),
    )
    ...
    leg, y = zip(
        ...
        (r'$\sqrt{2\ln 2}\tau$',
            brune2gauss(x, dt, tau, tau*np.sqrt(2 * np.log(2)))),
