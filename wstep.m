...
for id = 1:3
  ...
  for iz = 1:size( operator, 1 )
    ...
    k = i1(2):i2(2);
    l = i1(3):i2(3);
    s2(j,k,l) = dfnc( op, s1, x, h, 1, id, j, k, l );
  end
  op = operator{1,1};
  i1 = halo1 + 1;
  i2 = halo1 + ncore - 1;
  ...
  for i = 1:npml
    if id ~= 1 && bc(1), ji = i1(1) + i - 1;
      s2(ji,k,l) = dfnc( op, u, x, h, ic, id, ji, k, l );
    end
    if id ~= 1 && bc(4), ji = i2(1) - i + 1;
      s2(ji,k,l) = dfnc( op, u, x, h, ic, id, ji, k, l );
    end
    if id ~= 2 && bc(2), ki = i1(2) + i - 1;
      s2(j,ki,l) = dfnc( op, u, x, h, ic, id, j, ki, l );
    end
    if id ~= 2 && bc(5), ki = i2(2) - i + 1;
      s2(j,ki,l) = dfnc( op, u, x, h, ic, id, j, ki, l );
    end
    if id ~= 3 && bc(3), li = i1(3) + i - 1;
      s2(j,k,li) = dfnc( op, u, x, h, ic, id, j, k, li );
    end
  ...
  for i = 1:npml
    if id == 1 && bc(1), ji = i1(1) + i - 1;
      s2(ji,k,l) = dfnc( op, v, x, h, ic, id, ji, k, l );
    ...
    if id == 2 && bc(2), ki = i1(2) + i - 1;
      s2(j,ki,l) = dfnc( op, v, x, h, ic, id, j, ki, l );
    ...
    if id == 3 && bc(3), li = i1(3) + i - 1;
      s2(j,k,li) = dfnc( op, v, x, h, ic, id, j, k, li );
