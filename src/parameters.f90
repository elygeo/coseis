...
case( 'rfunc' );        read( str, *, iostat=io ) key, op, rfunc
case( 'tfunc' );        read( str, *, iostat=io ) key, op, tfunc
case( 'rsource' );      read( str, *, iostat=io ) key, op, rsource
case( 'tsource' );      read( str, *, iostat=io ) key, op, tsource
...
case( 'np' );           read( str, *, iostat=io ) key, op, np
...
case default
  ...
  case( '=', '+' )
    call pappend
    read( str, *, iostat=io ) &
      p%mode, p%field, p%ii, p%filename, p%nb, p%val, p%tfunc, p%period, p%x1, p%x2
  case default; io = 1
  end select
end select
