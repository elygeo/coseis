...
subroutine scalarbc( f, ibc1, ibc2, cell, nhalo )
real, intent(inout) :: f(:,:,:)
integer, intent(in) :: ibc1(3), ibc2(3), c, nhalo
...
! Mirror on cell BC
if ( cell == 0 ) then
  if ( i1(1) == 2 ) forall( i=1:nhalo ) f(j1-i,:,:) = f(j1+i-1,:,:)
  if ( i1(2) == 2 ) forall( i=1:nhalo ) f(:,k1-i,:) = f(:,k1+i-1,:)
  if ( i1(3) == 2 ) forall( i=1:nhalo ) f(:,:,l1-i) = f(:,:,l1+i-1)
  if ( i2(1) == 2 ) forall( i=1:nhalo ) f(j2+i,:,:) = f(j2-i+1,:,:)
  if ( i2(2) == 2 ) forall( i=1:nhalo ) f(:,k2+i,:) = f(:,k2-i+1,:)
  if ( i2(3) == 2 ) forall( i=1:nhalo ) f(:,:,l2+i) = f(:,:,l2-i+1)
else
