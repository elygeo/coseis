...
do j = 1, size( x, 1 ) 
  ...
  c   = ep2 * cf2
  aa  = (x(j,k,l)-l0) * cf
  m   = j1*y(j,k,l) + s2f*(j2 + c2f*(j3 + c2f*j4))
  x(j,k,l) = k0*n*aa*(1.+aa*aa/6.*(c+1-t2+aa*aa*.05*(5.+c*72.-ep2*58.+t2*(t2-18.))))
  y(j,k,l) = k0*(m+n*t*aa*aa*(.5+aa*aa/24.*(5.-t2+c*(c*4.+9.)+aa*aa/30.*(c*600.+61.-ep2*330.+t2*(t2-58.)))))
end do
end do
end do
x = x + 500000.
...
do j = 1, size( x, 1 ) 
  s2f = sin( y(j,k,l)*2. )
  c2f = cos( y(j,k,l)*2. )
  f1  = y(j,k,l) + s2f*(j1 + c2f*(j2 + c2f*(j3 + c2f*j4)))
  cf  = cos( f1 )
  sf  = sin( f1 )
  cf2 = cf * cf
  sf2 = sf * sf
  ...
  d   = x(j,k,l) / (n*k0)
  x(j,k,l) = l0+(d-d*d*d/6.*(1.+t2*2.+c-d*d*.05*(5.+ep2*8.-c*(c*3.+2.)+t2*(t2*24.+28.))))/cf
  y(j,k,l) = f1-n*t*r*d*d*(.5-d*d/24.*(5.+t2*3.-ep2*9.-c*(c*4.-10.)-d*d/30.*(61.-ep2*252.+t2*(t2*45.+90.)-c*(c*3.-298.))))
end do
end do
end do
x = x * 180. / pi
