...
for dx, np in runs:
    ...
    i1 = [j, k, -2, -1]
    prm['fieldio'] += [
        ['x1',   i0, 'w', 'x1.bin'],           # X coordinates
        ['x2',   i0, 'w', 'x2.bin'],           # Y coordinates
        ['su1',  i1, 'w', 'su1.bin'],          # final horizontal slip
        ['su2',  i1, 'w', 'su2.bin'],          # final vertical slip
        ['psv',  i1, 'w', 'psv.bin'],          # peak slip velocity
        ['trup', i1, 'w', 'trup.bin'],         # rupture time
    ]
    ...
    # run SORD
    prm['rundir'] = os.path.join('run', 'tpv3', '%03.0f' % dx)
