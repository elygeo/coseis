...
for path in glob.glob( sims ):
    ...
    # skip if still running or already cooked
    running = os.path.exists( path + 'in' )
    cooked = not os.path.exists( out + 'full-v1' )
    if cooked or running:
        print 'Skipping ' + path
        continue

    # lock file for concurrent processes
    try:
        lock = os.mkdir( path + 'lock' )
    except:
        print 'Skipping  ' + path
        continue
    print 'Cooking ' + path
    ...
    for it in range( nt ):
        ...
        # progress bar
        sord.util.progress( clock, it+1, nt, 'Cook' )
    ...
    # save pgv, pgd
    np.asarray( np.sqrt( pgv ), dtype ).T.tofile( path + 'pgv' )
    np.asarray( np.sqrt( pgd ), dtype ).T.tofile( path + 'pgd' )
    np.asarray( np.sqrt( pgvh ), dtype ).T.tofile( path + 'pgvh' )
    np.asarray( np.sqrt( pgdh ), dtype ).T.tofile( path + 'pgdh' )
