% TPV3
  np       = [    1   1  32 ];
  np       = [    1   2  32 ];
  nn       = [  701 401 256 ];
  n1expand = [    0   0   0 ];
  n2expand = [    0   0   0 ];
  bc1      = [   10  10  10 ];
  bc2      = [   -2   2  -2 ];
  ihypo    = [   -2  -2  -2 ];
  fixhypo  =     -2;
  xhypo    = [   0.  0.  0. ];
  affine   = [ 1. 0. 0.   0. 1. 0.   0. 0. 1. ];
  nt  = 6000;
  dx  =  25.;
  dt  = .002;

  rho = 2670.;
  vp  = 6000.;
  vs  = 3464.;
  gam = .2;
  gam = { .02 'cube' -15001. -7501. -4000.   15001. 7501. 4000. };
  hourglass = [ 1. 2. ];

  faultnormal = 3;
  vrup = -1.;
  dc  = 0.4;
  mud = .525;
  mus = 10000.;
  mus = { .677   'cube' -15001. -7501. -1.  15001. 7501. 1. };
  tn  = -120e6;
  ts1 = 70e6;
  ts1 = { 81.6e6 'cube'  -1501. -1501. -1.   1501. 1501. 1. };

  out = { 'x'     1   1  1 -2  0   -1 -1 -2  0 };
  out = { 'pv2'   1   1  1 -2 -1   -1 -1 -2 -1 };
  out = { 'vm2' 400   1  1 -2  0   -1 -1 -2 -1 };
  out = { 'vm2'   8   1  0 -2  0   -1  0 -2 -1 };
  out = { 'vm2'   8   0  1 -2  0    0 -1 -2 -1 };
  out = { 'tsm'   1   1  1  0  0   -1 -1  0  0 };
  out = { 'su'    1   1  1  0 -1   -1 -1  0 -1 };
  out = { 'psv'   1   1  1  0 -1   -1 -1  0 -1 };
  out = { 'trup'  1   1  1  0 -1   -1 -1  0 -1 };
  timeseries = { 'su' -7451.   -49. 0. };
  timeseries = { 'sv' -7451.   -49. 0. };
  timeseries = { 'ts' -7451.   -49. 0. };
  timeseries = { 'su'   -49. -5951. 0. };
  timeseries = { 'sv'   -49. -5951. 0. };
  timeseries = { 'ts'   -49. -5951. 0. };

