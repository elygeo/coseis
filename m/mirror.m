...
if ( i1(1) == 2 )
  s = sign( bc1(1) );
  f(j+1:2*j,:,:,:,:) = f;
  f(j:-1:1,:,:,:,1) = off1(1) - s * f(j+1:2*j,:,:,:,1);
...
if ( i1(2) == 2 )
  ...
  f(:,k:-1:1,:,:,2) = off1(2) - s * f(:,k+1:2*k,:,:,2);
...
if ( i1(3) == 2 )
  ...
  f(:,:,l:-1:1,:,3) = off1(3) - s * f(:,:,l+1:2*l,:,3);
end
if ( i1(3) == 3 )
  s = sign( bc1(3) );
  f(:,:,l:2*l-1,:,:) = f;
...
if ( i2(1) == 2 )
  s = sign( bc2(1) );
  i = size( f, 1 );
  f(i+j:-1:i+1,:,:,:,1) = off2(1) - s * f(i-j+1:i,:,:,:,1);
...
if ( i2(2) == 2 )
  ...
  f(:,i+k:-1:i+1,:,:,2) = off2(2) - s * f(:,i-k+1:i,:,:,2);
...
if ( i2(3) == 2 )
  ...
  f(:,:,i+l:-1:i+1,:,3) = off2(3) - s * f(:,:,i-l+1:i,:,3);
end
if ( i2(3) == 3 )
  s = sign( bc2(3) );
  i = size( f, 3 );
